...
// @lc code=start
const uint16 = new Uint16Array(510) 
...
function finalPrices(prices: number[]): number[] {
    ...
    return prices.map(
        (price, idx) => {
            const firstLtePrice = uint16.subarray(idx + 1, pricesSize).find(jPrice => jPrice <= price)
            return firstLtePrice !== undefined ? price - firstLtePrice : price
        }
    )
};
// @lc code=end
