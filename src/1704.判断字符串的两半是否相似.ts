...
function halvesAreAlike(s: string): boolean {
    const vowelCount = (s: string) => s.match(/([aeiou])/gi)?.length ?? 0
    const equalBy = <A, B>(fn: (it: A) => B) => (a: A, a2: A) => fn(a) === fn(a2)

    const half = s.length / 2
    return equalBy(vowelCount)(s.slice(0, half), s.slice(half))
}
// @lc code=end
