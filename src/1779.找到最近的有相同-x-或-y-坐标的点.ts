...
export {}
// @lc code=start
function nearestValidPoint(x: number, y: number, points: Array<[number, number]>): number {

    const getDistance = (point: [number, number]) => Math.abs(point[0] - x) + Math.abs(point[1] - y)

    return points
        .map(it =>
            x === it[0] || y === it[1] ? { _tag: "valid" as const, it } : { _tag: "invalid" as const }
        ).reduce<[min: number | null, ans: number]>(
            (acc, item, idx) => {
                if (item._tag === 'invalid') {
                    return acc
                } else {
                    const [min, ans] = acc
                    const dis = getDistance(item.it)
                    if (min === null) {
                        return [dis, idx]
                    } else {
                        return dis < min ? [dis, idx] : [min, ans]
                    }
                }
            },
            [null, -1]
