...
let largestMerge = (word1, word2) => {
    open Js
    let rec go = (ans, w, ww) => {
        if w->String2.length > 0 || ww->String2.length > 0 {
            let (add, w, ww) = if w->String2.localeCompare(ww) >= 0.0 {
                (w->String2.slice(~from=0, ~to_=1), w->String2.sliceToEnd(~from=1), ww)
            } else {
                (ww->String2.slice(~from=0, ~to_=1), w, ww->String2.sliceToEnd(~from=1))
            }
            go(ans ++ add, w, ww)
        } else {
            ans
