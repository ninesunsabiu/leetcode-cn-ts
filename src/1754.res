...
// @lc code=start
let largestMerge = (word1, word2) => {
  open Js
  let getHead = {String2.slice(~from=0, ~to_=1)}
  let getTail = {String2.sliceToEnd(~from=1)}
  let rec go = (ans, w, ww) => {
    open String2
    if w->length > 0 || ww->length > 0 {
      let (appendCh, w, ww) = if w->localeCompare(ww) >= 0.0 {
        (w->getHead, w->getTail, ww)
      } else {
        (ww->getHead, w, ww->getTail)
      }
      go(ans ++ appendCh, w, ww)
    } else {
      ans
    }
  }
