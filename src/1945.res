...
let getLucky = (s, k) => {
  open Js
  let getCode = {
    let charCodeAtZero = String2.charCodeAt(_, 0)
    let offset = "a"->charCodeAtZero
    s => {
      s->charCodeAtZero->\"-."(offset)->\"+."(1.0)
    }
  }

  let reduceDigitByCount = (s, count) => {
    let aux = s =>
      s
      ->String2.castToArrayLike
      ->Array2.fromMap(Float.fromString)
      ->Array2.reduce(\"+.", 0.)
      ->Float.toString
  ...
  s->String2.castToArrayLike->Array2.fromMap(getCode)->Array2.joinWith("")->reduceDigitByCount(k)
}
// @lc code=end
