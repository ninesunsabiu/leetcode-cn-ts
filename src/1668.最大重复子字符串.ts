...
function maxRepeating(sequence: string, word: string): number {
    const patternLen = word.length
    const maxK = sequence.length / patternLen | 0
    return (Array.from({ length: maxK }, (_, i) => word.repeat(maxK - i))
        .find(it => sequence.includes(it))?.length ?? 0) / patternLen
};
// @lc code=end
