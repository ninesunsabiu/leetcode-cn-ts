...
type Key = "type" | "color" | "name"
type Brand = string
type Color = string
type Name = string
type Item = [Brand, Color, Name]

const info = (rule: Key) => {
    return (item: Item): Brand | Color | Name => {
        if (rule === "type") {
            return item[0]
        } else if (rule === "color") {
            return item[1]
...
const match = (i: string) => (str: Brand | Color | Name) => str === i 

function countMatches(items: Array<Item>, ruleKey: Key, ruleValue: string): number {
    const getInfo = info(ruleKey)
    const matchTarget = match(ruleValue) 
    return items.filter(it => matchTarget(getInfo(it))).length
};
// @lc code=end
