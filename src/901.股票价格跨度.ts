...
export {}
// @lc code=start
type Span = [price: number, span: number] 
class StockSpanner {
    private spanArray: Array<Span> = []
    constructor() {}

    next(price: number): number {
        let ans: number = 1
        const array = this.spanArray;
        let idx = array.length - 1 | 0
        while (idx >= 0) {
            const [thePrice, theSpan] = array[idx]!
            if (price >= thePrice) {
                // 遇到一个 比 price 小的数
                // 则 它之前的跨度，全部都能给到当前跨度
                ans += theSpan
                // 下一次判断位置，从跨度之后的元素开始 因此 偏移量减去跨度
                idx -= theSpan
            } else {
                break;
            }
        }
        this.spanArray.push([price, ans])
        return ans
    }
}
